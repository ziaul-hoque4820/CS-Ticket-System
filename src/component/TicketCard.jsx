...
function TicketCard({ ticket, handleUpdateTaskStatus }) {
    return (
        ...
        >
            ...
            <p className="text-xs text-gray-500 mb-3 leading-relaxed line-clamp-2">{ticket.description}</p>
            <div className="flex items-center justify-between">
                <span className={`text-xs font-bold ${ticket.priority === "HIGH" ? "text-red-500" : ticket.priority === "MEDIUM" ? "text-yellow-500" : "text-green-500"}`}>{ticket.priority} PRIORITY</span>
                <span className="text-xs text-gray-400 font-medium bg-gray-50 px-2 py-0.5 rounded">{ticket.id}</span>
            </div>
